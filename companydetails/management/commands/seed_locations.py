...
from companydetails.models import Country, State, District

class Command(BaseCommand):
    help = 'Seed initial location data for India'

    def handle(self, *args, **options):
        india, created = Country.objects.get_or_create(name='India', code='IN')
        ...
        locations = {
            ...
            'Odisha': ['Angul', 'Balangir', 'Balasore', 'Bargarh', 'Bhadrak', 'Boudh', 'Cuttack', 'Deogarh', 'Dhenkanal', 'Gajapati', 'Ganjam', 'Jagatsinghpur', 'Jajpur', 'Jharsuguda', 'Kalahandi', 'Kandhamal', 'Kendrapara', 'Kendujhar', 'Khordha', 'Koraput', 'Malkangir', 'Mayurbhanj', 'Nabarangpur', 'Nayagarh', 'Nuapada', 'Puri', 'Rayagada', 'Sambalpur', 'Sonepur', 'Sundargarh'],
        ...
        for state_name, districts in locations.items():
            state, _ = State.objects.get_or_create(country=india, name=state_name)
            for dist_name in districts:
                District.objects.get_or_create(state=state, name=dist_name)
            self.stdout.write(self.style.SUCCESS(f'Successfully seeded {state_name}'))

        self.stdout.write(self.style.SUCCESS('Finished seeding all locations.'))
